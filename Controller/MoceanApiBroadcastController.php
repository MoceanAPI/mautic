<?php
namespace MauticPlugin\MoceanApiBroadcastBundle\Controller;
...
use MauticPlugin\MoceanApiBroadcastBundle\Form\Type\BroadcastType;
use MauticPlugin\MoceanApiBroadcastBundle\Entity\History;
use MauticPlugin\MoceanApiBroadcastBundle\Services\MoceanSms;

class MoceanApiBroadcastController extends FormController
{
    /**
    ...
    //Broadcast page views and functions
    public function broadcastAction(){
        ...
        foreach ($leads as $lead) {
            if($mobile = $lead->getMobile()) {
                if(!$country = $lead->getCountry())
                    $country = '';
                
                $leadOption = new \stdClass();
                $leadOption->mobile = $mobile;
                $leadOption->country = $country;
                $choices[$lead->getName()] = json_encode($leadOption);
            }
        }
        //Submit url
        $action = $this->generateUrl('mautic.moceanapi_broadcast.sms_broadcast');
        $form = $this->createForm(BroadcastType::class, [], ['action' => $action, 'choices' => $choices]);

        //Form submit event
        if ('POST' === $this->request->getMethod()) {
            $data = $this->request->request->get('broadcast');
            $message = $data['message'];
            $success = 0; 
            $total = 0;
            $error = false;

            //Get plugin configuration (account configuration)
            $this->integrationHelper = $this->get('mautic.helper.integration');
            $integration = $this->integrationHelper->getIntegrationObject('Mocean');
            ...
            if ($integration && $integration->getIntegrationSettings()->getIsPublished()) {
                ...
                $defaultCountryCode = $keys['country_code'];
                $sender = $keys['sender'];

                $request = new MoceanSms();
                ...
                if (!isset($keys['api_key']) || is_null($sender)) {
                    $this->addFlash(
                        'mautic.plugin.mocean.invalid_config',
                        [],
                        'notice'
                    );
                }
                //No valid recipient chosen                
                else if($data['recipient'] == 0) {
                    $this->addFlash(
                        'mautic.plugin.mocean.invalid_recipient',
                        [],
                        'notice'
                    );
                }
                //All contacts
                    ...
                    foreach ($leads as $lead) {
                        ...
                        if($recipient) {
                            ...
                            //Check for country code if enabled
                            if($defaultCountryCode) {
                                ...
                                if($checkedRecipient = $request->checkCountryCode())
                                    $recipient = $checkedRecipient;
                                else { //Invalid format for selected country
                                    $this->insertHistory($sender, $message, $recipient, 'Invalid format for mobile number', 'Fail', 'N/A', '');
                                    break;
                                }
                            }
                            $request->setRecipient($recipient);
                ...
                //Specific contacts
                else if($data['recipient'] == 2) {
                    foreach($data['specific_recipients'] as $lead) {
                        $total++;
                        $lead = json_decode($lead);
                        $recipient = preg_replace('/[^0-9]/', '', $lead->mobile);
                ...
                else if($data['recipient'] == 3) {
                    $specificNumber = $data['specific_numbers'];
                    $recipients = array();
                    $recipients = preg_split('~[^0-9]~i', $specificNumber, -1, PREG_SPLIT_NO_EMPTY);
                ...
                $this->addFlash(
                    'mautic.plugin.mocean.publish_reminder',
                    [],
                    'notice'
                );
            }
            ...
            if($error) 
                $this->failMessage($success, $total);
            else
                $this->successMessage($success, $total);
        }
        
        return $this->delegateView(
            [
                'contentTemplate' => 'MoceanApiBroadcastBundle:MoceanApiBroadcast:broadcast.html.php',
                'viewParameters' => [
                    'heading' => 'SMS Broadcast',
    ...
    //History page views and functions
    public function historyAction(){
        //Get list of sms transaction history
        $repository = $this->getDoctrine()->getRepository(History::class);
        $history = $repository->findAll();

        //Action url for export logs
        $action = $this->generateUrl('mautic.moceanapi_broadcast.export_logs');

        return $this->delegateView(
            [
                'contentTemplate' => 'MoceanApiBroadcastBundle:MoceanApiBroadcast:history.html.php',
                'viewParameters' => [
                    'heading' => 'SMS Transaction History',
                    'history' => $history,
                    'action' => $action
                ]
            ]
    ...
    //Get credit balance with currency
    public function getCreditBalance(){
        ...
        $request->setApiKey($apiKey);
        $request->setApiSecret($apiSecret);
        $jsonCredit = $request->getCredit();
        $jsonCurrency = $request->getPricing();
        
        if ($jsonCredit['status'] == 0 && $jsonCurrency['status'] == 0)
            $creditBalance = $jsonCurrency['destinations'][0]['currency'].' '.$jsonCredit['value'];
        else {
            if ($jsonCredit['status'] != 0) 
                $creditBalance = $jsonCredit['err_msg'];
            else if ($jsonCurrency['status'] != 0) 
                $creditBalance = $jsonCurrency['err_msg'];
        }
    ...
    public function insertHistory($sender, $message, $recipient, $response, $status, $logs1, $logs2){
        $history = new History();
        ...
        $history->setSmsLog1($logs1);
        $history->setSmsLog2($logs2);
    ...
    //Flash message for successful sms broadcast
    public function successMessage($success, $total) {
        $this->addFlash(
            'mautic.plugin.mocean.sent_sms_success',
            ['%success%' => $success, '%total%' => $total],
            'notice'
        );
    }

    //Flash message for failed sms broadcast
    public function failMessage($success, $total) {
        $this->addFlash(
            'mautic.plugin.mocean.sent_sms_fail',
            ['%success%' => $success, '%total%' => $total],
            'notice'
        );
    }

    //Call mocean sms api and return status of sms request
    public function sendSms(MoceanSms $request) {
        $jsonSms = $request->sendBroadcast();
        ...
        //varchar max 191 chars
        $logs = str_split($jsonSms['log'], 191);
        
        $this->insertHistory($request->getSender(), $request->getMessage(), $request->getRecipient(), $response, $status, $logs[0], $logs[1]);
        return $sent;
    }
    ...
    public function exportAction()
    {
        ...
        $query = $repository->createQueryBuilder('h')->select(['h.smsLog1', 'h.smsLog2'])->getQuery();
        $logs = $query->getArrayResult(); 
        
        foreach($logs as $log) {
            $data .= "{".$log['smsLog1'].$log['smsLog2']."},\n";
        }
        $data = rtrim($data, ",\n");
        ...
        $disposition = $response->headers->makeDisposition(
            ...
            "Mautic_MoceanApiBroadcastLog.json"
        );
    ...
    //Get country code for contacts with country in their address information
    public function getCountryCode($country) {
        ...
        $key = array_search($country, array_column($countries, 'name'));
